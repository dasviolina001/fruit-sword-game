...
document.addEventListener('touchmove', e => {
  if (e.touches.length > 0) {
    ...
    sword.style.left = touch.pageX - 40 + 'px';
    sword.style.top = touch.pageY - 40 + 'px';
  }
});
...
function createObject() {
  ...
  let startX = Math.random() * (window.innerWidth - 60);
  let startY = -60; // Start from top
  ...
  function move() {
    ...
    if (
      ...
      if (isBomb) {
        ...
        // Delay game over so the sound can finish
        setTimeout(() => {
          endGame();
        }, 800);
        obj.remove();
